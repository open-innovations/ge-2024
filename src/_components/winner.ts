...
export default function ({ winner, previous, results, parties, thumbnails, tag }: Lume.Data) {
	let html = '';
	let party = winner.party_key;
	let pts = Object.keys(parties);
	let key = pts.includes(party) ? party : "other";
	let c = Colour(key);
	let bg = c.hex;
	...
	html += '<span class="headline">' + winner.party_name + (previous == party ? ' HOLD':' GAIN') + '</span>';
